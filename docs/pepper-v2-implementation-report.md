...
2. LLM thought appears within ~1-2s
3. Click Pepper → New thought generated
4. Check `data/visitors/` for JSON files
5. Test without API key → Returns 503 (no fallback)

### Local Development
```bash
make start    # Start server in background, shows URL
make stop     # Stop server
make restart  # Restart server
make status   # Check if running
make logs     # Tail server logs
make dev      # Foreground mode (interactive)
```
...
- Removed instant greeting engine (Phase 1) - LLM-only now
- Removed hardcoded greetings that felt "bland"
- Simplified to 2-layer architecture: Visitor Memory + LLM

**Prompt Improvements:**
- Removed `timeOnPage` from page_load trigger (was always 0)
- Context now only includes relevant data per trigger
- Tighter system prompt: "be natural, not corny"

**Model Update:**
- Changed from fictional `gpt-5-nano` to `gpt-5.2`
- Removed unsupported `max_tokens` and `reasoning` params

**Developer Experience:**
- Added Makefile with `start`/`stop`/`restart`/`status`/`logs`
- Port read from `.env` or server log (not hardcoded)
- `.server.pid` and `.server.log` for process management

---

## Conclusion

Pepper v2 transforms a static mascot into an AI-powered companion. The simplified two-phase approach ensures:

1. **Persistent memory** - Returning visitors are recognized
2. **Personalized AI** - Context-aware, genuine interactions via LLM

Trade-off: No instant fallback if LLM is slow/down. Acceptable given gpt-5.2 latency (~1s).
